...
if (!isset($_SESSION)) {
    ...
    if (!isset($uid) || $uid === $_SESSION['uid']) {
        ...
        echo "<form autocomplete='off' action='./tools/upload_to_pdp.php' method='post' enctype='multipart/form-data'>";
    ...
    if (!isset($uid) || $uid === $_SESSION['uid']) {
        echo "<form class='addform' action='././main/imglab.php' method='none'>";
    ...
    echo "</div>";
    if (!isset($uid) || $uid === $_SESSION['uid']) {
        echo "<form  action='./tools/edit_user_info.php'  id='editdiv' class='divedit' method='POST'>";
        ...
        echo "<a id='pb' type='button' class='selectbut' value='Postes' href='./main/profile.php?postes=s'>Postes</a>";
        echo "<a id='lb' type='button' class='selectbut' value='Liked' href='./main/profile.php?liked=s'>Liked</a>";
        echo "<a id='sb'type='button'  class='selectbut' value='Saved' href='./main/profile.php?saved=s'>Saved</a>";
        echo "</div>";
    }
    if (isset($_GET['liked']))
        include './tools/showlimgs.php';
    else if (isset($_GET['saved']))
        include './tools/showsimgs.php';
    else
        include './tools/showpimgs.php';
}
