...
if (!empty($_POST['csrf']) && hash_equals($_SESSION['token'], $_POST['csrf'])) {
    ...
    try {
        ...
        if (isset($_POST["submit"])) {
            ...
            {
                matchpass($npw, $cnpw, "./forget_pw.php?key=".$key."&email=".$email);
                hardpwd($npw, "./forget_pw.php?key=".$key."&email=".$email);
                ...
                $db->exec($sql);
                header('location: ../login.php?sucs=true');
                return;
            }
            header('location: ../login.php?error=fixpassword');
        }
